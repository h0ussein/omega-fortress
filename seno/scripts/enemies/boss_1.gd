...
func _ready():
	...
	# Set max_health for elite enemy
	max_health = 600.0  # Higher health for this elite enemy
	...
	print("EliteEnemy: Ready with health: " + str(health) + "/" + str(max_health))

# Create a health bar for the elite enemy
func create_health_bar():
	# Create a ProgressBar node for the health bar
	var new_health_bar = ProgressBar.new()
	new_health_bar.name = "HealthBar"
	
	# Set size and position - make it larger for elite enemy
	...
	# Style the health bar - use a different color for elite enemy
	var style_box = StyleBoxFlat.new()
	style_box.bg_color = Color(0.9, 0.6, 0.1)  # Orange/gold for elite enemy
	...
	print("EliteEnemy: Created health bar")
	...
	if health_bar:
		...
		if style_box is StyleBoxFlat:
			if health_percent > 0.6:
				style_box.bg_color = Color(0.9, 0.6, 0.1)  # Orange/gold for healthy
			elif health_percent > 0.3:
				style_box.bg_color = Color(0.9, 0.4, 0.1)  # Darker orange for medium health
			else:
				style_box.bg_color = Color(0.9, 0.2, 0.2)  # Red for low health
