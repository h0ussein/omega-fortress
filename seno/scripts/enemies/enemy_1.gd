...
func _ready():
	...
	# Set max_health for enemy1 specifically
	max_health = 100.0  # Set the max health for enemy_1
	...
	print("Enemy1: Ready with health: " + str(health) + "/" + str(max_health))
...
func create_health_bar():
	...
	print("Enemy1: Created health bar")
...
func _physics_process(delta):
	...
	if target_hero and is_instance_valid(target_hero):
		...
		if path.size() > 0:
			...
			var raw_direction = global_position.direction_to(target_base.global_position)
			
			# IMPORTANT: Restrict to cardinal directions only
			var direction = Vector2.ZERO
			
			# Determine the dominant axis and only move along that axis
			if abs(raw_direction.x) > abs(raw_direction.y):
				# Move horizontally
				direction.x = sign(raw_direction.x)
			else:
				# Move vertically
				direction.y = sign(raw_direction.y)
