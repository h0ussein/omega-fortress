...
var barrier_queue: Array = []  # Queue of barriers to attack
...
func attack_barrier():
	...
	if not target_barrier or not is_instance_valid(target_barrier):
		...
		# Check if there are more barriers in the queue
		if barrier_queue.size() > 0:
			# Get the next barrier from the queue
			var next_barrier = barrier_queue.pop_front()
			
			# Make sure it's still valid
			if is_instance_valid(next_barrier):
				print("Enemy8: Targeting next barrier in queue")
				target_barrier = next_barrier
				
				# Continue attacking
				attack_barrier()
				return
		
		# No more barriers to attack, resume movement
	...
	if target_barrier.has_method("take_damage"):
		...
		if target_barrier != null and is_instance_valid(target_barrier):
			if target_barrier.has_signal("barrier_destroyed"):
				if not target_barrier.is_connected("barrier_destroyed", _on_barrier_destroyed):
					target_barrier.barrier_destroyed.connect(_on_barrier_destroyed)

# Add a barrier to the queue
func add_barrier_to_queue(barrier: Node2D):
	if not barrier or not is_instance_valid(barrier):
		return
	
	# Check if this barrier is already in the queue
	for existing_barrier in barrier_queue:
		if existing_barrier == barrier:
			return  # Already in queue
	
	# Check if this is the current target
	if target_barrier == barrier:
		return  # Already targeting
	
	# Add to queue
	barrier_queue.append(barrier)
	print("Enemy8: Added barrier to queue. Queue size: " + str(barrier_queue.size()))

# Sort barriers by distance (closest first)
func sort_barriers_by_distance():
	if barrier_queue.size() <= 1:
		return  # Nothing to sort
	
	barrier_queue.sort_custom(func(a, b):
	...
	print("Enemy8: Sorted barrier queue by distance")
...
func _on_attack_area_body_entered(body: Node2D) -> void:
	...
	print("Enemy8: Body entered attack area: " + body.name + " in groups: " + str(body.get_groups()))
	...
	# Check for base
		...
		print("Enemy8: Base entered attack range")
		move_speed = 0
		if animation:
			animation.play("attack1")
	...
	# Check for barriers
	elif body.is_in_group("barriers") or (body.get_parent() and body.get_parent().is_in_group("barriers")):
		var barrier = body if body.is_in_group("barriers") else body.get_parent()
		print("Enemy8: Barrier entered attack range: " + barrier.name)
		
		if target_barrier == null:
			# No current target, attack this barrier immediately
			target_barrier = barrier
			move_speed = 0
			if animation:
				animation.play("attack1")
			...
			# Already targeting a barrier, add this one to the queue
			add_barrier_to_queue(barrier)
			
			# Sort barriers by distance
			sort_barriers_by_distance()

func _on_attack_area_body_exited(body: Node2D) -> void:
	if is_dying or is_paused:
		return
	
	print("Enemy8: Body exited attack area: " + body.name)
	
	# Hero exited
	if body == target_hero:
		print("Enemy8: Hero exited attack range")
		target_hero = null
		
		# If we have a barrier target or queue, keep attacking barriers
		if target_barrier or barrier_queue.size() > 0:
			if not target_barrier and barrier_queue.size() > 0:
				target_barrier = barrier_queue.pop_front()
				attack_barrier()
		else:
			# No targets, resume movement
			move_speed = original_speed
			if animation:
				animation.play("run")
	...
	# Base exited
		...
		print("Enemy8: Base exited attack range")
	...
	# Barrier exited - remove from queue if it's in there
	elif body.is_in_group("barriers") or (body.get_parent() and body.get_parent().is_in_group("barriers")):
		var barrier = body if body.is_in_group("barriers") else body.get_parent()
		
		# Remove from queue if present
		for i in range(barrier_queue.size() - 1, -1, -1):
			if barrier_queue[i] == barrier:
				barrier_queue.remove_at(i)
				print("Enemy8: Removed barrier from queue: " + barrier.name)
				break
...
func _on_barrier_destroyed(barrier, grid_pos = null):
	if barrier == target_barrier:
		...
		# The destroyed barrier was in our queue, remove it
		for i in range(barrier_queue.size() - 1, -1, -1):
			if barrier_queue[i] == barrier:
				barrier_queue.remove_at(i)
				print("Enemy8: Removed destroyed barrier from queue")
				break
